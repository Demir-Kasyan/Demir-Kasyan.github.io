...
function formatLocale$2(locale) {
  var group = locale.grouping === undefined || locale.thousands === undefined ? identity$3 : formatGroup$1(map$3.call(locale.grouping, Number), locale.thousands + ""),
      currencyPrefix = locale.currency === undefined ? "" : locale.currency[0] + "",
      currencySuffix = locale.currency === undefined ? "" : locale.currency[1] + "",
      decimal = locale.decimal === undefined ? "." : locale.decimal + "",
      numerals = locale.numerals === undefined ? identity$3 : formatNumerals$1(map$3.call(locale.numerals, String)),
      percent = locale.percent === undefined ? "%" : locale.percent + "",
      minus = locale.minus === undefined ? "-" : locale.minus + "",
      nan = locale.nan === undefined ? "NaN" : locale.nan + "";
...
function defaultLocale(definition) {
  locale = formatLocale$2(definition);
...
function removeDuplicates$2(vertices) {
  for (var i = 0; i < vertices.length - 1; i++) {
    var v0 = vertices[i];
    var v1 = vertices[i + 1];
  ...
  _createClass$1(GeoPolygon, [{
    ...
    value: function set() {
      ...
      for (var i = 0; i < this.numPolygons; i++) {
        removeDuplicates$2(this.vertices[i]);

        if (this.vertices[i].length > 2) {
          close$1(this.vertices[i]);
        }

        this.polygons[i] = create$7({
...
reg$2.add('geopolygon', create$8);
reg$2.add('polyline', create$9);
/* eslint-disable import/prefer-default-export */

function create$a(type, input) {
  ...
  _createClass$1(GeometryCollection, [{
    ...
    value: function set() {
      ...
      collection.forEach(function (geo) {
        var geoInstance = create$a(geo.type, geo);
...
function inverseTransform(node, input) {
  ...
  if (Array.isArray(transformedInput.vertices)) {
    if (transformedInput.vertices.every(function (item) {
      ...
      transformedInput = create$8(transformedInput);
    } else {
  ...
  _createClass$1(DisplayObject, [{
    ...
    get: function get() {
      ...
      switch (this._collider.type) {
        case 'collection':
          this._collider.fn = create$b(this._collider.definition);
          break;
        ...
        case 'bounds':
          this._collider.fn = create$a('rect', this.boundingRect());
          break;
        ...
        case 'geopolygon':
        case 'polyline':
          this._collider.fn = create$a(this._collider.type, this._collider.definition);
          break;
...
var Path$1 = /*#__PURE__*/function (_DisplayObject) {
...
reg$3.add('rect', create$i);
reg$3.add('circle', create$j);
reg$3.add('text', create$m);
reg$3.add('line', create$k);
reg$3.add('path', create$l);
reg$3.add('stage', create$f);
reg$3.add('container', create$e);
reg$3.add('defs', create$e);
reg$3.add('linearGradient', create$g);
reg$3.add('radialGradient', create$g);
reg$3.add('stop', create$g);
reg$3.add('pattern', create$h);
/* eslint-disable import/prefer-default-export */

function create$n(type, input) {
...
function traverse(items, parent, matrix, on) {
  ...
  for (var i = 0, len = items.length; i < len; i++) {
    ...
    var displayNode = create$n(state.node.type, state.node);
...
function renderer$2() {
  ...
  dom.render = function (nodes) {
    ...
    if (Array.isArray(nodes)) {
      vNode = v("div", null, nodes);
    } else {
      vNode = nodes;
    }

    dNode = M(vNode, el, dNode);
    return true;
  };

  dom.renderArgs = [v]; // Arguments to render functions using the DOM renderer
...
components.forEach(p$1.use);
renderers.forEach(p$1.use);
scales.forEach(p$1.use);
